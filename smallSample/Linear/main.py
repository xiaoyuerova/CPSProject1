...
from smallSample.Linear.textLinear import *
import pandas as pd
import numpy as np
import gensim
from smallSample.Linear.m import BOW
import copy
...
def evaluate(evaluate_round: int, prop: str, dir_num=None):
    ...
    for epoch in range(EPOCH):
        ...
        print('acc in train: ',
              accuracy_score(label_train.cpu().data.numpy(), np.argmax(output.cpu().data.numpy(), axis=1)))
        output = model(x_test.cuda())
        label_test = y_test.to(dtype=torch.int64)
        allLabel = label_test.cpu().data.numpy()
        allPred = np.argmax(output.cpu().data.numpy(), axis=1)
